...
export default function App() {
  ...
  const [likedPizzaIds, setLikedPizzaIds] = useState<number[]>([]);
  ...
  const visible = category === "Under ₹500" ? pizzas.filter(p => p.price * 84 < 500) : pizzas.filter(p => p.category === category);
  ...
  function toggleFavorite(id: number) { setLikedPizzaIds(ids => ids.includes(id) ? ids.filter(item => item !== id) : [...ids, id]); setNotice(likedPizzaIds.includes(id) ? "Removed from favorites" : "Saved to favorites"); setTimeout(() => setNotice(""), 1600); }

  if (isLoading) return <div className="splash-screen"><div className="splash-content"><div className="splash-logo"><span className="brand-flame">✦</span><span>ember<br /><i>pizza</i></span></div><div className="pizza-loader"><div className="loader-pizza">🍕</div><span className="loader-ring ring-one" /><span className="loader-ring ring-two" /></div><p className="splash-copy">Firing up something good<span className="loading-dots">...</span></p><div className="splash-progress"><span /></div></div><span className="splash-note">WOOD-FIRED · WILDLY GOOD</span></div>;
  ...
  return <div className="app-shell">
    ...
    <main className="container main-content">
      {active === "Home" && <>
        ...
        <section className="menu-section" id="menu"><div className="section-heading"><div><p className="eyebrow">The good stuff</p><h2>Pick your <em>pie.</em></h2></div><button className="text-button" onClick={() => document.getElementById("menu")?.scrollIntoView({ behavior: "smooth" })}>View full menu <ArrowRight size={15} /></button></div>
          <div className="category-tabs">{["Popular", "Vegetarian", "Meat lovers", "Sides", "Drinks", "Under ₹500"].map(c => <button key={c} className={category === c ? "active" : ""} onClick={() => setCategory(c)}>{c}</button>)}</div>
          <div className="pizza-grid">{visible.map(pizza => <article className="pizza-card" key={pizza.id}><div className="pizza-image-wrap"><img src={pizza.image} alt={pizza.name} /><span className="pizza-tag">{pizza.tag}</span><button className={likedPizzaIds.includes(pizza.id) ? "heart liked" : "heart"} onClick={() => toggleFavorite(pizza.id)} aria-label={`Favorite ${pizza.name}`}><Heart size={17} fill={likedPizzaIds.includes(pizza.id) ? "currentColor" : "none"} /></button></div><div className="pizza-card-body"><div><h3>{pizza.name}</h3><p>{pizza.description}</p></div><div className="pizza-bottom"><span className="price">₹{pizza.price * 84}<small> / 12"</small></span><button className="add-button" onClick={() => add(pizza.id)}><Plus size={17} /> Add</button></div></div></article>)}</div>
        </section>
      </>}
